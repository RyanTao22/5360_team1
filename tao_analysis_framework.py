...
# Define the backtesting and indicator calculation functions
class QuantStrategy:
    ...
    def run(self, start_date, end_date):
        ...
        dates = [start_date_dt + timedelta(days=i) for i in range(num_days)]

        for date in dates:
            for second in range(23400):  # 6.5 hours * 3600 seconds
                random_value = random.randint(-10, 10) if second != 0 else 0
                self.networth.append(self.networth[-1] + random_value)
                
                current_time = date.replace(hour=9, minute=30, second=0) + timedelta(seconds=second)
                self.timestamp.append(current_time)

def calculate_indicators(net_worth_list, baseline_networth, initial_cash, timestamps, annual_risk_free_rate=0.04, n_loops_a_day=6.5 * 3600):
...
# Initialize the Dash app
    ...
    app.layout = html.Div([
        html.Div([
            # make the label at the center at y-axis(use top: 50%, absolute position, transform: translateY(-50%))
            html.Label('Start Date', style={'font-weight': 'bold','margin-bottom': '10px'}),
            dcc.Input(id='start_date', value='20240111', type='text', style={'margin-bottom': '10px'}),
            html.Label('End Date', style={'font-weight': 'bold'}),
            dcc.Input(id='end_date', value='20240112', type='text', style={'margin-bottom': '10px'}),
    ...
    def update_backtest(n_clicks, start_date, end_date, initial_cash):
        if n_clicks == 0:
            return ['--']*16 + [{}]

        # Run the backtest
        strategy = QuantStrategy(initial_cash)
        strategy.run(start_date, end_date)

        net_worth_list = strategy.networth[1:]
        timestamps = strategy.timestamp

        baseline_networth = initial_cash + np.cumsum([random.randint(-10, 10) for _ in range(len(strategy.networth) - 1)])

        results = calculate_indicators(net_worth_list, baseline_networth, initial_cash, timestamps)
        ...
        # Adjust timestamps to exclude non-trading periods
        adjusted_timestamps = []
        adjusted_net_worth_list = []
        for ts, nw in zip(timestamps, net_worth_list):
            if ts.time() >= datetime.strptime('09:30:00', '%H:%M:%S').time() and ts.time() <= datetime.strptime('16:00:00', '%H:%M:%S').time():
